...
| File | → PPTX |
|------|--------|
| [qos-architecture.md](qos-architecture.md) | `qos-architecture.pptx` — **DT100 input** · Fri meeting |
...
**Diagrams (A3):** labels from `qos-architecture.md` → `scripts/a3_aligned_render.py` → PNG → pptx (no Mermaid; build validates every step)
...
| You | Gluon |
|-----|--------|
| Edit `qos-architecture.md` or `manager-arch-vision-b6.md` | `./scripts/run-deck-build.sh` |
